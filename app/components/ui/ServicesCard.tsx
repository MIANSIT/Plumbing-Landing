...
export default function ServicesCard({ icon, heading, details, more, id }: CardProps) {
  const router = useRouter();

  const handleCardClick = () => {
    // Use the id directly (already has hyphens)
    router.push(`/services/${id}`);
  };

  const handleCallClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    window.location.href = "tel:01793229730";
  };

  return (
    <div 
      onClick={handleCardClick}
      className="group relative h-72 flex flex-col overflow-hidden rounded-3xl shadow-md hover:shadow-2xl hover:shadow-blue-900/15 transition-all duration-500 hover:-translate-y-2 cursor-pointer"
    >
      <Image
        src={icon}
        alt={heading}
      ...
      <div className="absolute bottom-0 left-0 right-0 px-5 pb-5 pt-3 bg-gradient-to-t from-blue-950/95 to-blue-900/80 backdrop-blur-sm translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out">
        ...
        <button onClick={handleCallClick}  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-400 text-white font-bold text-sm shadow-lg shadow-orange-500/30 active:scale-95 transition-all duration-200 cursor-pointer">
